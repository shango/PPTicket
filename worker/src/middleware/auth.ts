...
export async function authMiddleware(c: Context<{ Bindings: Env }>, next: Next) {
  ...
  const user = await c.env.DB.prepare('SELECT id, email, name, first_name, last_name, avatar_url, role, must_change_password, theme, created_at, last_login FROM users WHERE id = ?').bind(payload.sub).first<User>();
