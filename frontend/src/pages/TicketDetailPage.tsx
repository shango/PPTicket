...
export function TicketDetailPage() {
  ...
  // Same rules as the board: chords and typing are left alone, so these never
  // eat a keystroke meant for the comment box or the edit form.
  useEffect(() => {
    function onKeyDown(e: KeyboardEvent) {
      if (e.metaKey || e.ctrlKey || e.altKey) return;
      const el = e.target as HTMLElement | null;
      if (el && (el.isContentEditable || ['INPUT', 'TEXTAREA', 'SELECT'].includes(el.tagName))) return;
      if (e.key === 'e' && canEdit && !editing) { e.preventDefault(); setEditing(true); return; }
      if (e.key === 'Escape' && editing) { setEditing(false); setSaveError(''); return; }
      if ((e.key === 'b' || e.key === 'Escape') && !editing) { navigate('/board'); }
    }
    window.addEventListener('keydown', onKeyDown);
    return () => window.removeEventListener('keydown', onKeyDown);
  }, [canEdit, editing, navigate]);
  ...
  return (
    <div className="h-full flex flex-col">
      <PageHeader
        title={`PDO-${ticket.ticket_number}`}
        backTo="/board"
        actions={
          <>
            ...
            {canEdit && !editing && (
              <button onClick={() => setEditing(true)} title="Edit (e)"
        ...
        <div className="max-w-5xl mx-auto px-6 py-6">
          ...
          <div className="mb-6">
            <div className="flex items-center gap-2.5 mb-2">
              {/* The ticket number lives in the page header now. */}
